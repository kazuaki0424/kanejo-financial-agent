...
import { signInWithEmail, signInWithGoogle } from '@/app/(auth)/_actions/auth';
  ...
  return (
    <Card>
      {message === 'confirmation_sent' && (
        <div className="mb-4 rounded-[var(--radius-md)] bg-info-bg px-4 py-3 text-sm text-info">
          確認メールを送信しました。メール内のリンクをクリックしてアカウントを有効化してください。
        </div>
      )}

      {errorParam && (
        <div className="mb-4 rounded-[var(--radius-md)] bg-negative-bg px-4 py-3 text-sm text-negative">
